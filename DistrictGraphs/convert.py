...
def blocks_frame_graph(blocks, geoid):
    ...
    with progressbar.ProgressBar(max_value=blocks.shape[0]) as bar:
        for (_, block) in progressbar.progressbar(blocks.iterrows()):
            bar.update()
    ...
    logger.debug(f'Finding intersections...')
    ...
    with progressbar.ProgressBar(max_value=intersections.shape[0]) as bar:
        for (id1, block1) in progressbar.progressbar(intersections.iterrows()):
            if _candidates % 100 == 0:
                bar.update()
            _candidates += 1
    ...
    logger.debug(f'Out of {_possibles:,d} possible connections,'
        f' reduced {_candidates:,d} candidates to {_connections:,d} contiguous pairs'
        f' in {_elapsed:.3f} seconds.')

    # Find all nodes that touch the outside
    node_ids = list(graph.nodes)

    with progressbar.ProgressBar(max_value=len(node_ids)) as bar:
